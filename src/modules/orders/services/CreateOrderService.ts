...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (checkInexistentProducs.length) {
      throw new AppError(
        `Could not find product(s) ${checkInexistentProducs.forEach(
          inexistentproduct => `id: ${inexistentproduct.id},`,
        )}`,
      );
    }
    ...
    if (findProductsWithNonQuantityAvailable.length) {
      throw new AppError('Some products dont have the requested quantity');
    }
